...
public class WxPayServiceImplTest {
  ...
  /**
   * https://github.com/search?l=Java&o=desc&q=USERPAYING&s=indexed&type=Code&utf8=%E2%9C%93
   * 添加内嵌式重试机制, 最终报出失败
   * @throws Exception
   */
  @Test
  public void testMicropay() throws Exception {
    WxPayMicropayResult result = this.payService.micropay(WxPayMicropayRequest.newBuilder()
            .body("body")
            .outTradeNo(RandomUtils.getRandomStr())
            .totalFee(100000)
            .spbillCreateIp("127.0.0.1")
            .authCode("134613917008530139" +
                    "" +
                    "")
            .build());
    this.logger.info(result.toString());
